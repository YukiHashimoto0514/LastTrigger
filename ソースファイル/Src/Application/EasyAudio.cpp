...
public:
  ...
  bool Init(ComPtr<IXAudio2> xaudio, IMFAttributes* attributes, const wchar_t* filename) {
    ...
    if (!xaudio || FAILED(xaudio->CreateSourceVoice(&sourceVoice, pWaveFormatEx))) {
      return false;
    }
...
public:
  ...
  */
  SoundPtr Prepare(const wchar_t* filename) {
    ...
    if (!xaudio || FAILED(xaudio->CreateSourceVoice(&sound->sourceVoice, &wf.u.ext.Format))) {
      return nullptr;
    }
  ...
  */
  SoundPtr PrepareMFStream(const wchar_t* filename) {
    ...
    if (!xaudio || !mfs->Init(xaudio, attributes.Get(), filename)) {
      return nullptr;
    }
